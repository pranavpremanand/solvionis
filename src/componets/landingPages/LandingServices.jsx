...
const LandingServices = ({ page }) => {
  ...
  return (
    <div id="services" className="flex justify-center relative bg-background">
      <div className="wrapper py-10 flex flex-col items-center gap-5 z-10">
        ...
        >
          {services.map((item) => (
            <div
              key={item.id}
              className="shadow-2xl transition-all border-2 border-primary/50 bg-primary/20 hover:-translate-y-1 hover:bg-primary/25 duration-300 rounded-lg"
            >
              <div className="flex h-full flex-col gap-2 items-center text-center p-5 rounded-lg">
                <item.icon className="w-[6rem] h-[6rem] fill-primary" />
                <h6 className="font-semibold text-2xl font-raleway mt-3">
                  {item.title}
                </h6>
                <p className="text-md desc">{item.description}</p>
              </div>
            </div>
          ))}
        </div>
